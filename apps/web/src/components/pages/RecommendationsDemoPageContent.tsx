...
import { Sparkles, TrendingUp, User, BarChart3, Trash2 } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="relative z-10 container mx-auto px-4 py-12">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">Test and explore the AI-powered event recommendation engine</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <button onClick={() => handleSimulateInteraction('view')} className="px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 text-blue-300 rounded-lg transition-colors">
                + View Event
              </button>
              <button onClick={() => handleSimulateInteraction('favorite')} className="px-4 py-2 bg-pink-500/20 hover:bg-pink-500/30 border border-pink-500/30 text-pink-300 rounded-lg transition-colors">
                + Favorite Event
              </button>
              <button onClick={() => handleSimulateInteraction('purchase')} className="px-4 py-2 bg-green-500/20 hover:bg-green-500/30 border border-green-500/30 text-green-300 rounded-lg transition-colors">
                + Purchase Event
              </button>
              <button onClick={() => handleSimulateInteraction('share')} className="px-4 py-2 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/30 text-purple-300 rounded-lg transition-colors">
                + Share Event
              </button>
              <button
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white/5 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-blue-400">{stats?.byType.view || 0}</p>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <p className="text-gray-400 text-sm mb-1">Favorites</p>
                <p className="text-3xl font-bold text-pink-400">{stats?.byType.favorite || 0}</p>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <p className="text-gray-400 text-sm mb-1">Purchases</p>
                <p className="text-3xl font-bold text-green-400">{stats?.byType.purchase || 0}</p>
              </div>
            </div>

            {userProfile && Object.keys(userProfile.preferences.categories).length > 0 && (
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-white mb-3">Category Preferences</h3>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(userProfile.preferences.categories)
                    .sort(([, a], [, b]) => b - a)
                    .map(([category, score]) => (
                      <div key={category} className="px-3 py-1 bg-purple-500/20 border border-purple-500/30 rounded-full">
                        <span className="text-purple-300 capitalize">{category}</span>
                        <span className="text-purple-400 ml-2 text-sm">{score.toFixed(1)}</span>
                      </div>
                    ))}
                </div>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-8">
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6 mb-6">
            ...
            <p className="text-gray-300 mb-4">Select an event below to see similar events based on content similarity:</p>
            <div className="flex flex-wrap gap-2">
          ...
          <SimilarEvents currentEvent={selectedEvent} allEvents={sampleEvents} limit={4} />
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6">
            ...
            <div className="grid md:grid-cols-3 gap-4 text-gray-300">
              <div>
                <h3 className="text-lg font-semibold text-white mb-2">Content-Based Filtering</h3>
                <p className="text-sm">Recommends events based on your category preferences, price range, and location history.</p>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-white mb-2">Collaborative Filtering</h3>
                <p className="text-sm">Finds users with similar preferences and recommends events they enjoyed using cosine similarity.</p>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-white mb-2">Popularity & Recency</h3>
                <p className="text-sm">Boosts trending events and applies time decay to keep recommendations fresh and relevant.</p>
              </div>
            </div>

    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="relative z-10 container mx-auto px-4 py-12">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">Monitor Core Web Vitals and application performance metrics</p>
        </motion.div>
        ...
        {uniqueMetricNames.length > 0 ? (
            ...
            {uniqueMetricNames.map((metricName, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`${getRatingColor(rating)} backdrop-blur-sm border rounded-lg p-6`}
                >
                  <div className="flex items-center justify-between mb-4">
                    <Icon className="w-8 h-8" />
                    <span className="px-3 py-1 text-xs font-semibold uppercase rounded-full bg-white/10">{rating}</span>
                  </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-12 text-center">
            ...
            <p className="text-gray-300">Navigate through the app to collect performance metrics. Web Vitals data will appear here automatically.</p>
          </motion.div>
        )}

        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mt-12">
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-white mb-4">Implemented Optimizations</h3>
